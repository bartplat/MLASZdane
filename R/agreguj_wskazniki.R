...
agreguj_wskazniki = function(wskazniki, grupy, przekazArgumenty = NULL, ...) {
  ...
  for (i in 1:nrow(grupy)) {
    ...
    assign(".data",
           ...
           envir = grupaEnv)
    if (!is.null(przekazArgumenty)) {
      nazwy_g = names(przekazArgumenty)
      for (p in 1:length(przekazArgumenty)) {
        assign(nazwy_g[p],
               przekazArgumenty[[p]],
               envir = grupaEnv)
      }
    ...
    if (!is.null(przekazArgumenty)) {
      nazwy_go = names(przekazArgumenty)
      for (q in 1:length(przekazArgumenty)) {
        assign(nazwy_go[q],
               przekazArgumenty[[q]],
               envir = odniesienieEnv)
      }
